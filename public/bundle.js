(function () {
	...
	var typer = {
		...
		stop: function stop() {
			...
			var response = this.current.join(""); 
			this.reset();
			this.complete(response);
		},
		onComplete: function onComplete(cb, trigger) {
	...
	var TypeWriter = {
	    ...
	    computed: Object.assign({}, classes,
	    ...
	    methods: {
	    	...
	    	startTalking: function startTalking() {
	    		...
	    		this.model.content.reduce(this.inSequence, Promise.resolve()).then(function () { return this$1.$emit('section-complete'); });
	    	},
	    	inSequence: function inSequence(chain, item) {
	    		var this$1 = this;

	    		return chain.then(function () {
	    			return new Promise(function (resolve) {
	    				this$1.typer.start(item, function (p) { return resolve(this$1.triggerNext(p)); });
	    			})    			
	    		})
	    	},
		    triggerNext: function triggerNext(addChat) {
		    	if(addChat) {
	...
	var store = [];
	Object.keys(Icon).forEach(function (icon) { return store.push(Icon[icon]); });
	...
	var Chatbot = {
	    name: "chatbot",
	    template: template$2,
	    components: { TypeWriter: TypeWriter, UserChoices: UserChoices },
	    props: [],
	    data: function data() {
	        return {
	        	actions: [],
	    ...
	    computed: Object.assign({}, classes$2,
	        {getCurrentContext: function getCurrentContext() {
	        	...
	        	return graph.chatbot.filter(function (item) { return item.id === this$1.context; }).pop()
	        },
	        getCurrentActions: function getCurrentActions() {
	        	return this.actions.reduce(function (accum, id) {
	        		var action = graph.actions.filter(function (item) { return item.id === id; }).pop();
	        		accum.push(action);
	        		return accum
	        	}, [])
	        }}),
	    methods: {
	    	addChoices: function addChoices(update) {
	    	...
	    	updateContext: function updateContext(context) {
	    		...
	    		this.context = context;
	    		this.$nextTick(function () { return this$1.handleTriggers(); });
	    	},
	        choiceComplete: function choiceComplete() {
	            var this$1 = this;

	            var to = this.getCurrentContext.timeout;
	            var n = to.next.toString();
	            if(to.next.length > 0) { setTimeout(function () {
	                var nxt = this$1.getCurrentContext.timeout.next.toString();
	                if(n === nxt) { this$1.updateContext(to.next); } 
	            }, to.duration); }
	        },
	        handleTriggers: function handleTriggers() {
	            var features = this.getCurrentContext.triggers;
	...
	var skillsCollection = ['Javascript', 'ES6', 'Node', 'React', 'Vue', 'Express', 'Firebase', 'AWS Lambda','NPM', 'CLI', 'Electron', 'Git Flow', 'CSS3', 'HTML5', 'Design'];
	var linksCollection = [{
		label: "Github",
		tag: "personal",
		url: "https:github.com/vonKristoff"
	},{
		label: "CV",
		tag: "personal",
		...
		label: "A Museum Made Digital",
		url: "http://explore.soane.org",
		tag: "portfolio"
	}, {
		label: "Paxman.js",
		tag: "portfolio",
		url: "http://bite-software.co.uk/paxman"
	}, {
		label: "Archive",
		tag: "personal",
	...
	var app = {
		el: "#root",
		components: { Chatbot: Chatbot, Skills: Skills },
		data: {
			preloader: store,
			feature: false
		},
		methods: {
			handleFeature: function handleFeature(features) {
				var f = features[0];
	...
	new (Function.prototype.bind.apply( Vue, [ null ].concat( app) ));
